...
workers.performCheck = function(originalCheckData){
    ...
    // Instantiate request object (using either the http or https module)
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome){
    ...
    console.log(checkOutcome.error);
    var state = !checkOutcome.error &&  checkOutcome.responseCode && originalCheckData.successCodes.indexOf(checkOutcome.responseCode) > -1 ? 'up' : 'down';

    // Decide if an alert is warranted
    var alerWarranted = originalCheckData.lastChecked && originalCheckData.state != state ? true : false;
...
workers.alertUserToStatusChange = function(newCheckData){
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function(err){
        if(!err){
            ...
            console.log("Could not send sms alert to user who state change in their check");
        }
    });
};

// Log workers
workers.log = function(originalCheckData,checkOutcome,state,alerWarranted,timeOfCheck){
    // Form the log data
    var logData = {
...
// Timer to execute the log rotation prcess once per day
...
workers.init = function() {
    ...
    // Compress all the logs immmediately
    workers.rotateLogs();
