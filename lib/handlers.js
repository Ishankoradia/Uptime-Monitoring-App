...
// This the Dashboard view of Checks
        ...
        _data.read('users', phone, function(err, userData){
            ...
            if(!err && userData){
                ...
                if(hashedPassword == userData.hashedPassword){
                    ...
                    performance.mark('creatting data for token');                    
                    ...
                    _data.create('tokens',tokenId,tokenObject,function(err){
                        performance.mark('finished storing token');

                        // Log out all the measurements
                        const perfObserver = new PerformanceObserver((items) => {
                            items.getEntries().forEach((m) => {
                                debug('\x1b[33m%s\x1b[0m',m.name+' '+m.duration);
                            })
                          })
                          
                        perfObserver.observe({ entryTypes: ["measure"], buffer: true });   

                        // Gather all the perfomance measurements
                        performance.measure('Beginning to end', 'entered function', 'finished storing token');
                        ...
                        performance.measure('Password Hashing', 'beginning password hashing', 'finished password hashing');
                        performance.measure('Token Data creation', 'creatting data for token', 'beginning storing token');
                        performance.measure('Token Data creation', 'beginning storing token', 'finished storing token');
